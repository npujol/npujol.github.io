...
id: devops-configuration-management-puppet
aliases: [ ]
tags:
\- roadmap
\- devops
\- devops-configuration-managemen
\- ready
\- online
\- --

```
# devops-configuration-management-puppe
...
1. The `exec` resource creates a new virtual machine using `virt-install`.
   The `creates` parameter ensures that the script only runs if the VM image doesn't already exist.
1. The `package` resource installs Postgres on the VM using `apt`.
1. The `file` resource configures the Postgres configuration file to listen on all interfaces and sets some basic settings. The `template` function uses an ERB template (`postgresql/config.erb`) to generate the
   configuration file content.
1. Finally, the `service` resource restarts the Postgres service to apply
