...
\_\_Roadmap info from [ roadmap website ] (<https://roadmap.sh/linux/disks-filesystems/swap>) \_\_

## Linux Swap under Disks Filesystems

Swap
space in Linux is used when the amount of physical memory (RAM) is full. If the system needs more memory resources and the physical memory is full, inactive pages in memory are moved to the swap space. Swap space is a portion of a hard disk drive (HDD) that is used for virtual memory.
...
1. In a regular file within an existing filesystem.
