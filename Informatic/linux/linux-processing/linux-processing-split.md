...
\_\_Roadmap info from [ roadmap website ] (<https://roadmap.sh/linux/text-processing/split>) \_\_
...
that
is
used, as the name suggests, to split large files into smaller files. The `split` command in Linux divides a file into multiple equal parts, based on the lines or bytes specified by the user.
